...
public abstract class Vehicle {
	...
	public Vehicle(Road currentRoad, ForwardLane sourceLane, Road destinationRoad,
		...
		//this.mapHeight.addListener((obs,oldVal, newVal) -> { this.region.setY(this.region.getY() - (oldVal.doubleValue() + newVal.doubleValue())/2) ; });
	...
	private void forwardLaneVehicleUpdation(Vehicle nextVehicle, boolean isLastVehicle) {
				...
				if(isLastVehicle) {
					if(trafficLight.isRed()) {
						double gap = (trafficLight.getRegion().getY() - trafficLight.getRegion().getHeight()/2) - (this.getY() + this.getHeight()/2);
							if(Compare.isLarger(gap , Vehicle.trafficLightPadding))
					...
					if(nextVehicleSpeed > 0) {	
						if(Compare.isSmallerEquals(gap ,Vehicle.vehiclePadding))
							this.setVelocityY(nextVehicleSpeed);
						
						else if(Compare.isLarger(gap, Vehicle.vehiclePadding)) 
					...
					else if(Compare.isEqual(nextVehicleSpeed,0)) {
		...
		case BOTTOM_TO_TOP:
			if(isLastVehicle) {
				...
				if(Compare.isLarger(nextVehicleSpeed,0)) {	
					if(Compare.isSmallerEquals(gap , Vehicle.vehiclePadding))
						this.setVelocityY(-nextVehicleSpeed);
		...
		case LEFT_TO_RIGHT:
			if(isLastVehicle) {
				if(trafficLight.isRed()) {
					double gap = (trafficLight.getRegion().getX() - trafficLight.getRegion().getWidth()/2) - (this.getX() + this.getWidth()/2);
				...
				if(Compare.isLarger(nextVehicleSpeed,0)) {	
					if(Compare.isEqual(gap ,Vehicle.vehiclePadding)|| Compare.isSmaller(gap, Vehicle.vehiclePadding))
						this.setVelocityX(nextVehicleSpeed);
					
					else if(Compare.isLarger(gap, Vehicle.vehiclePadding)) 
						this.setVelocityX(Vehicle.maxSpeed);
					
					else this.setVelocityX(0);  // Not required as the following condition gap >= padding must hold at all times  but still used as emergency brake 
		...
		case RIGHT_TO_LEFT:
			if(isLastVehicle) {
				if(trafficLight.isRed()) {
					double gap =  (this.getX() - this.getWidth()/2)  - (trafficLight.getRegion().getX() + trafficLight.getRegion().getWidth()/2);
						if(Compare.isLarger(gap , Vehicle.trafficLightPadding))
							this.setVelocityX(-Math.min(Vehicle.maxSpeed, gap - trafficLightPadding));
						else this.setVelocityX(0);
				}
